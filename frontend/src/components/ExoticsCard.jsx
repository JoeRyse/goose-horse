...
export default function ExoticsCard({ exoticTickets = {}, dailyDoubles = [] }) {
  ...
  return (
    <div className="space-y-6 font-sans">
      {/* Header Banner */}
      <div className="bg-white rounded-2xl p-5 border border-stone-200 shadow-xs print:p-0 print:border-none print:shadow-none">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-rose-50 border border-rose-200 flex items-center justify-center text-[#881337] text-lg font-black shrink-0 print:hidden">
            🎟️
          </div>
          <div>
      ...
      {!hasAnyTickets ? (
        <div className="bg-white rounded-2xl p-8 border border-stone-200 text-center font-mono text-xs text-stone-500 shadow-xs">
        ...
        <div className="space-y-5">
          {/* Daily Doubles */}
          {ddList.length > 0 && (
            <div className="bg-white rounded-2xl p-5 border border-stone-200 shadow-xs space-y-3 print:border-slate-400 print:shadow-none">
          ...
          {pk3List.length > 0 && (
            <div className="bg-white rounded-2xl p-5 border border-stone-200 shadow-xs space-y-3 print:border-slate-400 print:shadow-none">
              <h3 className="text-xs font-mono font-extrabold text-[#881337] uppercase tracking-wider flex items-center gap-2">
                <Sparkles className="w-4 h-4 text-[#881337] print:hidden" /> SUGGESTED PICK 3 TICKETS ({pk3List.length})
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 font-mono text-xs">
                {pk3List.map((ticket, idx) => (
                  <div
                    key={idx}
                    className="p-3 rounded-xl bg-rose-50/60 border border-rose-200 text-slate-900 font-bold leading-relaxed"
                    dangerouslySetInnerHTML={{ __html: typeof ticket === 'string' ? ticket : JSON.stringify(ticket) }}
                  />
                ))}
              </div>
